...
- Interruption retries on the socket, `tmux`, and SSH paths are now defined in
  ...
  already had, so repeated signals cannot extend a bounded wait. The remaining
  blocking calls on those paths are recorded with the reason each one propagates
  an interruption instead of retrying.
