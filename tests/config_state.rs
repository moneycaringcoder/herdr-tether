...
#[test]
fn state_v5_migration_records_no_pane_and_keeps_the_session_scoped_reading() {
    let temp = tempdir().unwrap();
    let path = temp.path().join("state.json");
    // A version 5 record for a running workload. No older Tether recorded which
    // pane it launched, and the migration must not guess: a session's only pane
    // is usually the launched one, and "usually" is the assumption the field
    // exists to remove. A wrong pane id would read as a workload that has gone.
    let source = r#"{"version":5,"sessions":[{"id":"tether-0197f198000070008000000000000001","host":"local","target":"local","directory":"/srv/app","preset":null,"command":"exec shell","tmux_session_id":7,"ownership_proof":"0197f198-0000-7000-8000-000000000099","status":"running","created_at":"2026-01-01T00:00:00Z","last_used_at":"2026-01-01T00:00:00Z","closed_at":null}],"orchestration_groups":[]}"#;
    fs::write(&path, source).unwrap();
    let store = StateStore::new(path.clone());
    ...
    assert_eq!(migrated.version, State::CURRENT_VERSION);
    assert_eq!(
        migrated.sessions[0].tmux_pane_id, None,
        "a migrated record has no pane, so it keeps being judged as a whole session"
    );
    assert_eq!(
        migrated.sessions[0].tmux_session_id,
        Some("$7".parse().unwrap())
