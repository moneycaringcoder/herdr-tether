...
The state machine is always host → resource. The resource list contains active owned records, current external catalog rows, and **Create new Tether workload**. Owned or external selection proceeds directly to placement and returns a distinct exact-ID resume or exact-name attach intent; Create proceeds through directory → command → placement. On an owned row only, press-only lowercase `c` enters a typed red confirmation and only `y` emits an exact `SessionId` close request; external/Create identities cannot represent destruction. Close executes on a worker. Once confirmed, Cancel/Back/selection are suppressed until its bounded result. Success removes exactly that owned row and refreshes both generations; failure remains retryable. Global press-only uppercase `P` separately requests an asynchronous persistence-only prune preview from any normal stage; lowercase `p` remains directory path entry. A nonempty preview opens a red count/retention confirmation that states **No host contact**. Read-only preview may be cancelled; after `y`, exit/selection is suppressed until apply returns. Apply success reports exact removed/skipped counts without changing active/external rows or restarting status/discovery. Preview/apply failures retain sanitized bounded retry state. Partial create CLI arguments suppress owned/external choices so they cannot be silently ignored.
