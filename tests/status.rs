    ...
    let run = service
        .try_start(StatusRequest {
            ...
            hosts: vec![StatusHost {
                ...
                workloads: vec![probe(clean), probe(failed), probe(unreadable), probe(alive)],
            }],
        })
        .unwrap();
...
case "$target" in
  duplicate) printf 'work:0:0:\nwork:1:0:\n' ;;
  unsafe) printf 'good:0:0:\nbad:name:0:0:\n' ;;
  *) exit 99 ;;
esac
"#,
    )
...
case "$target" in
  offline) exit 255 ;;
  empty) exit 1 ;;
  malformed) printf 'tether-not-an-id:0:0:\n' ;;
  *) exit 99 ;;
esac
"#,
    )
